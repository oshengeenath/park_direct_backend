...
//         error: "Slot already booked for the specified date and time.",
//       });
//     }
...
//     if (!parkingSlot) {
//       return res.status(400).json({
//         error: "Invalid slot number. Please provide a valid slot number.",
//       });
//     }
...
//     for (let i = startTimeIndex; i < startTimeIndex + 2; i++) {
//       // Loop for 2 slots (59 minutes)
//       if (parkingSlot.schedule[i] === "booked") {
//         return res
//           .status(400)
//           .json({ error: "Slot already booked at the specified time." });
//       }
//     }
...
app.post("/book-slot", async (req, res) => {
  const { email, date, startTime, carNumber } = req.body;

  try {
    // Check if a booking already exists for the specified date and start time for any car
